...
app.post("/payPostpaidInvoice", async (req, res) => {
  const { customerMail, planId, invoiceId, changePlan } = req.body;
  ...
  try {
    ...
    if (changePlan) {
      ...
      // Update customer plan's dates
      let customerPlan = await prisma.customerPlan.findUnique({
        where: {
          customerId_planId: {
            customerId: customer.customerId,
            planId: customer.customerCurrPlan,
          },
        },
      });
    ...
    // Respond with the success message after invoice payment
    res.status(200).json({
...
app.post("/buyPlan", async (req, res) => {
  ...
  try {
    ...
    // Determine plan instance and due date
    if (planType === "PREPAID" && plan.prepaidPlans.length > 0) {
      planInstance = plan.prepaidPlans[0];
      dueDateSet = new Date(now);
      dueDateSet.setDate(now.getDate() + 30); // Default to 30 days
    } else if (planType === "POSTPAID" && plan.postpaidPlans.length > 0) {
      planInstance = plan.postpaidPlans[0];
      dueDateSet = new Date(now);
      const billingCycleDays = parseInt(planInstance.billingCycle);
      ...
      dueDateSet.setDate(now.getDate() + billingCycleDays);
    } else {
      return res.status(404).json({ error: "Plan instance not found" });
    }
    ...
    // Create the CustomerPlan
    const customerPlanData = {
      customerId: customer.customerId,
      planId: plan.planId,
      activationDate: now.toISOString(),
      dueDate: dueDateSet.toISOString(),
    };

    let customerPlan = await prisma.customerPlan.create({ data: customerPlanData });
    ...
    // update the customer with the email id customerCurrPlan this value to the plan id
    await prisma.customer.update({ where: { customerMail: customerMail }, data: { customerCurrPlan: plan.planId } });
    // Once the payment is successful, create the invoice
    let invoiceData = {
      customerName: customer.customerName,
      customerId: customer.customerId,
      planId: plan.planId,
      date: now,
      planType: planType,
      units: planType === "PREPAID" ? planInstance.unitsAvailable : planInstance.unitsUsed,
      amount: planType === "PREPAID" ? planInstance.prepaidBalance : 0,
      status: planType === "PREPAID" ? "paid" : "N/A",
      stripeInvoiceId: session.id, // Save the Stripe session ID as invoice ID
    };
...
// app.post("/fetchPlan", async (req, res) => {
//   const { customerMail, planId, planType, invoiceId, changePlan } = req.body;
//   let plan, planInstance;
...
//       return res.status(404).json({ error: "Plan not found" });
//     }
//     planInstance = plan.postpaidPlans.find(plan => plan.planId === planId);
//     // Create Stripe Checkout Session
//     const session = await stripe.checkout.sessions.create({
//       payment_method_types: ['card'],
//       line_items: [
...
//               name: plan.planName,
//             },
//             unit_amount: (planInstance.unitsUsed*plan.ratePerUnit*100) || 0, // Replace with the actual amount
//           },
//           quantity: 1,
//         },
//       ],
//       mode: 'payment',
//       success_url: `http://localhost:3000/invoice`, // Redirect to invoice page or use invoiceId if available
//       cancel_url: `http://localhost:3000/ViewHistory`,
//     });
...
//       date: now,
//       planType: planType,
//       units: planInstance.unitsUsed,
//       amount: planInstance.unitsUsed,
//       status: "paid",
//     };
    
//     // Respond with the sessionId and invoice information
//     res.status(201).json({
//       message: "Payment done successfully for postpaid user",
//       planInstance,
//       sessionId: session.id, // Include sessionId in response
//       invoice: invoiceId,
//     });
    
//     await prisma.invoice.update({ where: { invoiceId: invoiceId }, data: { status: "paid" } });
